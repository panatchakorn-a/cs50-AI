...
class NimAI():
    ...
    def best_future_reward(self, state):
        ...
        """
        best_reward = 0
        for keys, values in self.q.items():
            if keys[0] == tuple(state) and values > best_reward:
                best_reward = values
    ...
    def choose_action(self, state, epsilon=True):
        ...
        """
        if epsilon:
            greediness = random.random() # randomly decide whether to random
            if greediness > self.epsilon:
                return self.choose_action_greedily(state)
            else:
